...
namespace Interactive360
{
    ...
    public class RaycastInput : MonoBehaviour
    {
        ...
        //[SerializeField] is used to initialize a new form object in Unity  

        //Check Edit -> Project Settings -> Input for mapping for different VR devices
        //Refer to https://docs.unity3d.com/Manual/OpenVRControllers.html for controller mappings

        [SerializeField] string oculusPrimaryInputAxis = "PrimaryIndexTrigger";   //The name of the input axis used to determine input for Oculus
        [SerializeField] string oculusSecondaryInputAxis = "SecondaryIndexTrigger";
        ...
        void Start()
        {
            ...
            Debug.Log("RaycastInput Started");
        ...
        void CheckInput()
        {
            ...
            //Debug.Log("The current primary input value is "+Input.GetAxis(oculusPrimaryInputAxis));
            //Debug.Log("The current secondary input value is " + Input.GetAxis(oculusSecondaryInputAxis));
            //If we press the primary input axis...
            if ((Input.GetAxis(oculusPrimaryInputAxis) >= 0.1 || Input.GetAxis(oculusSecondaryInputAxis) >= 0.1 ) && eventData.pointerEnter != null)
            {
        ...
        //This method converts a RaycastHit data type that we get from a raycast into a RaycastResult type that is used by the event system
        RaycastResult ConvertRaycastHitToRaycastResult(RaycastHit hit)
        {
            // Debug.Log("Object Hit with current input value: " + Input.GetAxis(oculusPrimaryInputAxis));
